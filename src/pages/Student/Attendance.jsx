...
const Attendance = ({ user, myClasses }) => {
  const [attendanceSummary, setAttendanceSummary] = useState([]);
  const [loading, setLoading] = useState(true);
  const studentID = user?.details?.StudentID;

  useEffect(() => {
    if (!studentID || !myClasses) {
      setLoading(false);
      return;
    }
    const classIdToSubjectMap = new Map();
    myClasses.forEach(cls => classIdToSubjectMap.set(cls.ClassID, cls.SubjectName || "Unknown Subject"));
  ...
  // --- NEW: Calculate overall totals from the per-subject summary ---
  ...
  return (
    <div className="content-card">
      ...
      {loading ? (
        <p style={{ textAlign: "center", padding: "20px" }}>Calculating attendance...</p>
      ) : (
        <>
