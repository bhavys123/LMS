...
const DashboardHome = ({ myClasses, user }) => {
  ...
  useEffect(() => {
    ...
    const unsubAssignments = onValue(assignmentsRef, (snapshot) => {
      ...
      const relevantAssignments = Object.values(allAssignments).filter(
        (a) =>
          studentClassIDs.has(a.classID) && // Use the Set for an efficient check
          a.status === 'ACTIVE' &&
    ...
    // --- 2. Fetch and Calculate Overall Attendance (Corrected Logic) ---
    ...
    const unsubAttendance = onValue(attendanceRef, (snapshot) => {
        ...
        Object.values(allData).forEach(dateData => {
            ...
            Object.entries(dateData).forEach(([classID, classData]) => {
                // FIX: Check if the attendance record is for the current student AND
                // if the class is one of the student's CURRENT classes.
                if(classData[studentID] && studentClassIDs.has(classID)) {
                    totalHeld++;
                    if (classData[studentID].status === 'Present' || classData[studentID].status === 'Late') {
                        totalPresent++;
                    }
                }
            });
      ...
      <div className="grid-3" style={{gap: '25px', gridTemplateColumns: '2fr 1fr'}}>
        ...
        <div style={{display: 'flex', flexDirection: 'column', gap: '25px'}}>
            <div className="content-card">
                ...
                ) : <p style={styles.placeholder}>🎉 No classes scheduled for today.</p>}
            </div>

            <div className="content-card">
                <div className="card-header"><span className="card-title">Upcoming Assignments</span></div>
                ...
                ) : <p style={styles.placeholder}>No upcoming assignments.</p>}
            </div>
        </div>
        
        {/* Right Column for Attendance */}
        <div className="content-card">
            <div className="card-header"><span className="card-title">Overall Attendance</span></div>
                ...
                {overallAttendance.loading ? <p>Loading...</p> : (
                    <>
                        ...
                        <p style={styles.attendanceText}>
                            Your attendance for current subjects is <strong>{Math.round(overallAttendance.percentage)}%</strong>.
                            {overallAttendance.percentage < 75 && " Please attend classes more regularly."}
                        </p>
                        <small style={{textAlign: 'center', color: '#94a3b8', display: 'block'}}>
                            Click on the 'Attendance' tab for a detailed subject-wise report.
                        </small>
                    </>
                )}
            </div>
